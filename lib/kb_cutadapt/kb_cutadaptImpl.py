...
class kb_cutadapt:
    ...
    ######################################### noqa
    VERSION = "1.1.0"
    GIT_URL = "https://github.com/kbaseapps/kb_cutadapt.git"
    GIT_COMMIT_HASH = "90b214b1783bfb8128003fb3aac24320a5a45c4c"
    ...
    def __init__(self, config):
        ...
        if self.config['SDK_CALLBACK_URL'] is None:
    ...
    def remove_adapters(self, ctx, params):
        ...
        for arg in required_params:
            if arg not in params or params[arg] is None or params[arg] == '':
        ...
        # output object
        if exec_remove_adapters_retVal['output_reads_ref'] is not None:
            reportObj['objects_created'].append(
                {'ref': exec_remove_adapters_retVal['output_reads_ref'],
                 'description': 'Post Cutadapt Reads'})
        else:
            raise ValueError("no output generated by exec_remove_adapters()")

        # save report object
        report = KBaseReport(self.config['SDK_CALLBACK_URL'], token=ctx['token'],
                             service_ver=SERVICE_VER)
        report_info = report.create(
            {'report': reportObj, 'workspace_name': params['output_workspace']})
    ...
    def exec_remove_adapters(self, ctx, params):
        ...
        token = ctx['token']
        wsClient = Workspace(self.config['workspace-url'], token=token)
        ...
        try:
            ...
            [OBJID_I, NAME_I, TYPE_I, SAVE_DATE_I, VERSION_I, SAVED_BY_I, WSID_I, WORKSPACE_I,
             CHSUM_I, SIZE_I, META_I] = list(range(11))

            input_reads_obj_info = \
            wsClient.get_object_info_new({'objects': [{'ref': params['input_reads']}]})[0]
            ...
            raise ValueError('Unable to get read library object from workspace: (' + str(
                params['input_reads']) + ')' + str(e))

        acceptable_types = ["KBaseSets.ReadsSet", "KBaseRNASeq.RNASeqSampleSet",
        ...
        if input_reads_obj_type not in acceptable_types:
            raise ValueError(
                f"Input reads of type: '{input_reads_obj_type}'.  "
                f"Must be one of {', '.join(acceptable_types)}")
        ...
        if "KBaseSets.ReadsSet" in input_reads_obj_type:
            ...
            sample_set = ws.get_objects2(
                {"objects": [{"ref": params['input_reads']}]}
            )["data"][0]["data"]
            sample_refs = list()
        ...
        for reads_item_i, input_reads_library_ref in enumerate(readsSet_ref_list):
            ...
            if (input_reads_obj_type != "KBaseSets.ReadsSet" and
                ...
                exec_remove_adapters_OneLibrary_params['output_object_name'] = params[
                    'output_object_name']
            else:
                exec_remove_adapters_OneLibrary_params['output_object_name'] = \
                    readsSet_names_list[reads_item_i] + "_cutadapt"
            ...
            for group in list(optional_g_params.keys()):
            ...
            msg = f"\n\nRUNNING exec_remove_adapters_OneLibrary() ON LIBRARY: "
            msg += f"{input_reads_library_ref} {readsSet_names_list[reads_item_i]}\n"
            msg += "----------------------------------------------------------------------------\n"
            report += msg
            self.log(console, msg)

            # RUN
            exec_remove_adapters_OneLibrary_retVal = \
            self.exec_remove_adapters_OneLibrary(ctx, exec_remove_adapters_OneLibrary_params)[0]
        ...
        # ReadsSet or SampleSet
            ...
            if some_cutadapt_output_created:
                ...
                cutadapt_readsSet_ref = \
                setAPI_Client.save_reads_set_v1({'workspace_name': params['output_workspace'],
